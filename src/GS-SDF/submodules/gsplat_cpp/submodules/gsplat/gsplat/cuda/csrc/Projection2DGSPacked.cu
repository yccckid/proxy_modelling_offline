...
__global__ void projection_2dgs_packed_fwd_kernel(
    ...
    scalar_t *__restrict__ randns,         // [nnz, 2]
    scalar_t *__restrict__ samples         // [nnz, 3]
) {
    ...
    if (valid) {
        ...
        scales += col_idx * 3;
        RS_world =
            quat_to_rotmat(glm::make_vec4(quats)) *
            mat3(scales[0], 0.0, 0.0, 0.0, scales[1], 0.0, 0.0, 0.0, 1.0);
        mat3 RS_camera = R * RS_world;
        ...
        //         float cutoff = 3.0f;
        // #endif
        //         const vec3 temp_point =
        //             vec3(cutoff * cutoff, cutoff * cutoff, -1.0f);
    ...
    if (block_cnts != nullptr) {
        ...
        if (valid) {
            ...
            // sample point
            vec3 sample = RS_world[0] * randns[thread_data * 2] +
...
void launch_projection_2dgs_packed_fwd_kernel(
    ...
    at::optional<at::Tensor> randns,         // [nnz, 2]
    at::optional<at::Tensor> samples         // [nnz, 3]
) {
    ...
    projection_2dgs_packed_fwd_kernel<float>
        <<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
            ...
            randns.has_value() ? randns.value().data_ptr<float>() : nullptr,
            samples.has_value() ? samples.value().data_ptr<float>() : nullptr
        );
}
...
__global__ void projection_2dgs_packed_bwd_kernel(
    ...
    const scalar_t *__restrict__ randns,         // [nnz, 2]
    // grad outputs
    ...
    const scalar_t *__restrict__ v_samples,        // [nnz, 3]
    ...
    vec3 v_sample = glm::make_vec3(v_samples);
    vec2 randn = glm::make_vec2(randns);
    v_mean += v_sample;
    mat3 R_gs = quat_to_rotmat(quat);
    ...
    quat_to_rotmat_vjp(quat, v_R_gs, v_quat);
