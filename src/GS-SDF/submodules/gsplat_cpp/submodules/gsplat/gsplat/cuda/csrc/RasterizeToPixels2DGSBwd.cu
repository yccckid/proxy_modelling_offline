...
__global__ void rasterize_to_pixels_2dgs_bwd_kernel(
    ...
    const scalar_t *__restrict__ render_colors, // [C, image_height,
                                                // image_width, CDIM]
    const scalar_t
        *__restrict__ render_depths, // [C, image_height, image_width, 1]
    const scalar_t
        *__restrict__ render_alphas,        // [C, image_height, image_width, 1]
    const scalar_t *__restrict__ render_Ts, // [C, image_height, image_width, 2]
    const int32_t
    ...
    const scalar_t
        *__restrict__ v_render_colors, // [C, image_height, image_width,     //
                                       // RGB CDIM]
    const scalar_t *__restrict__ v_render_depths, // [C, image_height,
                                                  // image_width, 1]  // depths.
    const scalar_t
...
#pragma unroll
    ...
    float final_B = inside ? render_Ts[pix_id] : 0.0f;
    float final_C = inside ? render_Ts[pix_id + 1] : 0.0f;
    if (v_render_distort != nullptr) {
        ...
        accum_d_buffer = render_depths[pix_id];
...
#pragma unroll
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size;
             ...
             * ==================================================
             */
            float depth; // ray-splat hit point in camera space
...
#pragma unroll
                ...
                if (v_render_distort != nullptr) {
                    ...
                    // float m = depth;
                    float dm_ddepth =
                        (far_n * near_n) / ((far_n - near_n) * depth * depth);

                    const float dl_dm =
                        2.0f * fac * (m * final_A - final_C) * v_distort;
                    v_depth += dl_dm * dm_ddepth;
                    // v_depth += 2.0f * fac * (m * final_A - final_C) *
                    // v_distort;
...
#pragma unroll
                ...
                /**
                 * Update the cumulative "later" gaussian contributions, used in
                 * derivatives of render with respect to alphas
                 */
                buffer_depths += depth * fac;

/**
 * Update the cumulative "later" gaussian contributions, used in
 * derivatives of output normals w.r.t. alphas
 */
#pragma unroll
                for (uint32_t k = 0; k < 3; ++k) {
...
void launch_rasterize_to_pixels_2dgs_bwd_kernel(
    ...
    const at::Tensor render_depths, // [C, image_height, image_width, 1]
    const at::Tensor render_alphas, // [C, image_height, image_width, 1]
    const at::Tensor render_Ts,     // [C, image_height, image_width, 2]
    const at::Tensor last_ids,      // [C, image_height, image_width]
    const at::Tensor median_ids,    // [C, image_height, image_width]
    // gradients of outputs
    const at::Tensor v_render_colors,  // [C, image_height, image_width, 3]
    const at::Tensor v_render_depths,  // [C, image_height, image_width, 1]
